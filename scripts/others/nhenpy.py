...
class NHentaiDoujin():
    ...
    def labels(self):
        ...
        artist = []
        extra = []
        for info in self._labels:
            info: str()
            if info.startswith('/tag/'):
                tags.append(info[5:][:-1])
            elif info.startswith('/language/'):
                language.append(info[10:][:-1])
            elif info.startswith('/category/'):
                category.append(info[10:][:-1])
            elif info.startswith('/artist/'):
                artist.append(info[8:][:-1])
            else:
                extra.append(info)

        grand_info = {}
        grand_info["tags"] = tags
        grand_info["language"] = language
        grand_info["category"] = category
        grand_info["artist"] = artist
        grand_info["extra"] = extra
